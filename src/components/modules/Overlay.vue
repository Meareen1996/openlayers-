...
export default {
  ...
  methods: {
    ...
    initMap() {
      ...
      this.addSymbolMarkers();
      this.getPoint();
      this.singleClickFun();
    },

    /**
     * 获取经纬度feature(需求:找到某个经纬度的点,动态设置样式)
     */
    getPoint() {
      let position = [116.48107607733336, 39.88376327014636];
      let feature = this.vectorLayer
        .getSource()
        .getFeaturesAtCoordinate(position);
      console.log("找到的feature", feature);
      if (feature) {
        if (feature.length > 0) {
          var style = feature[0].getStyle();
          // 重新设置图标的缩放率，基于层级10来做缩放
          console.log(style[0].getText());
